...
def test_validate_input_size_within_limits() -> None:
    s = Settings(
        jwt_secret="x" * 12,
        workflow=WorkflowSettings(max_user_input_chars=1000, max_user_estimated_tokens=500),
    )
...
def test_validate_input_size_too_many_chars() -> None:
    s = Settings(
        jwt_secret="x" * 12,
        workflow=WorkflowSettings(max_user_input_chars=10, max_user_estimated_tokens=500),
    )
...
def test_validate_input_size_estimated_tokens() -> None:
    s = Settings(
        jwt_secret="x" * 12,
        workflow=WorkflowSettings(max_user_input_chars=100_000, max_user_estimated_tokens=2),
    )
